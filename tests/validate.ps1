...
Assert-Contains $script 'PromptExitChoice\(\*\)' "Script should show an exit choice dialog from the main window"
Assert-Contains $script 'Minimize to Tray' "Exit choice should offer minimizing to tray"
Assert-Contains $script 'Exit PeekDock' "Exit choice should offer fully exiting PeekDock"
...
Assert-NotContains $script '--app="' "Script should preserve the original Chrome window instead of opening an app duplicate"
Assert-NotContains $script '--user-data-dir="' "Script should not create a separate Chrome profile for the dock"
...
Assert-Contains $script 'RememberAppWindow\(boundHwnd\)' "Binding should remember the current Chrome window instead of opening a duplicate"
Assert-Contains $script 'No bound Chrome window is available' "Toggle should ask the user to rebind instead of opening a duplicate page"
Assert-NotContains $script 'LaunchAppWindow\(url\)' "Toggle should not launch a new Chrome page for an already bound URL"
...
Assert-Contains $buildInstaller 'AutoHotkey64\.exe' "Installer build script should bundle the AutoHotkey runtime"
Assert-Contains $install 'runtime' "Installer should install a bundled runtime directory"
Assert-Contains $install 'runtimeTarget' "Installer should use the bundled AutoHotkey runtime"
Assert-Contains $install 'Installer payload is missing AutoHotkey64\.exe' "Installer should fail clearly if the bundled runtime is missing"
Assert-NotContains $install 'AutoHotkey.AutoHotkey' "Installer should not depend on winget for AutoHotkey v2"
Assert-NotContains $install 'AutoHotkey/AutoHotkey/releases/latest' "Installer should not download AutoHotkey during setup"
